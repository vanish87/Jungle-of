...
//         ofDrawBitmapString("vel "+ ofToString(butterfly_vel_.x) + " "+ ofToString(butterfly_vel_.y), 20, 80);
	...
	void GamingScene::keyPressed( int key )
	{
	...
	void GamingScene::mouseMoved( int x, int y )
	{           
		hand_pos_.x = x;
		hand_pos_.y = y;
	}

	void GamingScene::windowResized( int w, int h )
	{
        w_ = w;
        h_ = h;
	}

	void GamingScene::inPose( ofxUser & user )
	{
		cout<<ofGetElapsedTimef()<<"Pose---"<<user.pose<<endl;
		
	}

	void GamingScene::inGesture( ofxUser & user )
	{
		cout<<ofGetElapsedTimef()<<"Gesture---"<<user.gesture<<endl;		
	}

	void GamingScene::userIn( ofxUser & user )
	{
	}

	void GamingScene::userMove( ofxUser & user )
	{
	}

	void GamingScene::userOut( ofxUser & user )
	{
	}
